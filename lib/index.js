...
const getEnabledEngines = (configFilePath) => {
  ...
  configYaml.engines.forEach((engine) => {
    const attrs = configYaml.engines[engine];
  ...
  provideLinter() {
    ...
    return {
      ...
      lint: async (textEditor) => {
        ...
        const linterResults = [];
        let range;
        Object.values(messages).forEach((issue) => {
          if (Object.prototype.hasOwnProperty.call(issue.location, 'positions')) {
            ...
            if (issue.location.positions.begin.column !== undefined) {
              ...
              if (issue.location.positions.end.column !== undefined) {
                // Valid end column, generate full range
                const colEnd = issue.location.positions.begin.column - 1;
